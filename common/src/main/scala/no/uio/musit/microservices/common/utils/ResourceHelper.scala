...
object ResourceHelper {
  ...
  def postRootWithMusitResult[A](servicePostCall: A => Future[Either[MusitError, A]], musitResultToPost: Either[MusitError, A], toJsonTransformer: A => JsValue) = {
    musitResultToPost.mapToFinalPlayResult { objectToPost => postRoot(servicePostCall, objectToPost, toJsonTransformer) }
  }
  ...
  def updateRootWithMusitResult[A](serviceUpdateCall: (Long, A) => Future[Either[MusitError, MusitStatusMessage]], id: Long, musitResultObjectToUpdate: Either[MusitError, A]): Future[Result] = {
    musitResultObjectToUpdate.mapToFinalPlayResult { objectToUpdate => updateRoot(serviceUpdateCall, id, objectToUpdate) }
  }
