...
//import no.uio.musit.microservice.storagefacility.domain.event.{ Event, MusitEvent }
//import no.uio.musit.microservice.storagefacility.domain.event.{ RelatedEvents, _ }
//import no.uio.musit.microservice.storagefacility.domain.event.dto.BaseEventDto
//import no.uio.musit.microservice.storagefacility.service._ // FIXME: This is basically one of the deadly sins
...
//   */
//  def insertEventAction(
//    event: MusitEvent,
//    partialEventLink: Option[PartialEventLink],
//    recursive: Boolean
//  ): DBIO[Long] = {
//    def copyEventIdIntoLinks(eventBase: MusitEvent, newId: Long) =
//      event.baseEvent.links.getOrElse(Seq.empty).map(l => l.copy(localTableId = Some(newId)))
//
...
//      newEventId <- insertBaseAction(event.baseEventDto.copy(partOf = partOfParent))
...
//
//  /**
//   * Gets the subevents from the event with id=parentId from the database.
...
//        val result = MusitFuture.traverse[MusitFuture[Event], Event](groupOfMusitResultEvents)(identity)
//        result
//      }
//
