...
class KdReportDao @Inject() (  val dbConfigProvider: DatabaseConfigProvider
  ...
  def getReportTotalArea: Future[MusitResult[Int]] = {
    db.run(
      sql"""
         select SUM(area_to)
         from MUSARK_STORAGE.STORAGE_NODE
         WHERE storage_type = 'Room'
      """.as[Int].head.map(MusitSuccess.apply)
    ).recover {
...
def getAreaPerimeterSecurity: Future[MusitResult[Int]] = {
  db.run(
  sql"""
         select SUM(area_to)
  ...
  def getAreaTheftProtection: Future[MusitResult[Int]] = {
    db.run(
      sql"""
         select SUM(area_to)
         from MUSARK_STORAGE.STORAGE_NODE s, MUSARK_STORAGE.ROOM r
  ...
  def getAreaFireProtectiony: Future[MusitResult[Int]] = {
    db.run(
      sql"""
         select SUM(area_to)
         from MUSARK_STORAGE.STORAGE_NODE s, MUSARK_STORAGE.ROOM r
  ...
  def getAreaWaterDamageAssessment: Future[MusitResult[Int]] = {
    db.run(
      sql"""
         select SUM(area_to)
         from MUSARK_STORAGE.STORAGE_NODE s, MUSARK_STORAGE.ROOM r
  ...
  def getAreaRoutinesAndContingencyPlan: Future[MusitResult[Int]] = {
    db.run(
      sql"""
         select SUM(area_to)
         from MUSARK_STORAGE.STORAGE_NODE s, MUSARK_STORAGE.ROOM r
